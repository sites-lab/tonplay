  ...
  for (const userDoc of usersSnap.docs) {
    ...
    for (const depDoc of depositsSnap.docs) {
      ...
      if (msg && msg.trim().length >= 10) {
        await sendBotMessageWithButton(
          chatId,
          msg,
          lang === 'ru' ? '🎮 Открыть TonPlay' : '🎮 Open TonPlay',
          `https://t.me/${BOT_USERNAME}/${APP_PATH}`
        );
      } else {
        console.warn(`runWeeklyYield: skipping malformed message for user ${userId}`);
      }
...
// Minimum length a valid payment comment must have (our codes are longer than 3 chars)
...
function extractComment(tx) {
  ...
  if (msgData['@type'] === 'msg.dataText') {
    try {
      ...
      const cleaned = text.replace(/^\x00+/, '').trim();
      // Reject empty or too-short results (e.g. a stray ".", "·", single chars)
      if (!cleaned || cleaned.length < MIN_COMMENT_LENGTH) return null;
      return cleaned;
    } catch(e) { return null; }
  }
  if (msgData['@type'] === 'msg.dataRaw') {
    try {
      ...
      const cleaned = bytes.slice(4).toString('utf8').trim();
...
async function sendBotMessageToUser(chatId, text) {
  // Guard: chatId must be a valid positive integer (Telegram user ID)
  const numericId = parseInt(chatId);
  if (isNaN(numericId) || numericId <= 0) {
    console.warn(`sendBotMessageToUser: skipping invalid chatId "${chatId}"`);
    return;
  }
  // Guard: message must be a non-empty, meaningful string (not just dots/whitespace/punctuation)
  if (!text || typeof text !== 'string' || text.trim().length === 0) {
    console.warn('sendBotMessageToUser: skipping empty or invalid message text');
    return;
  }
  // Guard: reject messages that are suspiciously short (likely a bug artifact)
  if (text.trim().length < 10) {
    console.warn(`sendBotMessageToUser: skipping suspiciously short message: "${text.trim()}"`);
    return;
  }
  try {
    await fetch(`https://api.telegram.org/bot${BOT_TOKEN}/sendMessage`, {
